...
public class OrderDaoImpl implements OrderDao {
	 ...
	 * createdate.getDayOfMonth(); // 일 추출 (1부터 31까지의 값) String yearString =
	 * Integer.toString(year); String subYearString = yearString.substring(2,4);
	 * //년도 뒷자리2개만 추출 String monthString =Integer.toString(month); String dayString
	 * =Integer.toString(day);
	 * 
	 * Random random = new Random(); // 5자리 난수 생성 int min = 10000; // 최소값 (5자리 숫자)
	 * int max = 99999; // 최대값 (5자리 숫자) int randomNumber = random.nextInt(max - min
	 * + 1) + min;
	 * 
	 * //년도(뒷두자리)+월+일+randomNumber(5자리)
	 * order.setOrderNo(subYearString+monthString+dayString+randomNumber);
	 * 
	 * 
	 * 
	 * 
	 ...
	 * 
	 * for (OrderItem orderItem : itemList) { orderItem.setOrders(order);
	 ...
	 * } return null; }
	 * 
	 * @Override public List<Orders> findOrdersByUserName(Long memberId) { // Member
	 * findMember= memberRepository.findByMemberId(memberId); //memberRepository에
	 * findbyMemberId 메서드 추가
	 * 
	 * // return orderRepository.findByMemberIdCode(findMember.getMemberIdCode);
	 * 
	 * return null; }
	 * 
	 * @Override public List<Orders> findOrdersWithOrderItemByMemberId(Long
	 * memberId) { // 주어진 memberId를 사용하여 해당 회원의 주문 목록을 조회하고, 주문 항목을 함께 로드하려면 적절한 로직을
	 * 추가 // orderRepository와 JOIN 또는 FetchType.LAZY를 사용하여 관련된 주문 항목을 함께 로드 return
	 * null; }
	 * 
	 * @Override public Orders findOrdersByorderNo(Long oNo) { // 주어진 주문 번호 (oNo)를
	 * 사용하여 해당 주문을 조회하려면 적절한 로직을 추가 // orderRepository를 사용하여 데이터베이스에서 주문을 조회 return
	 * orderRepository.findById(oNo).get(); }
	 * 
	 * @Override public Orders findOrdersByOrderNoAndNameAndPhoneNo(String orderNo,
	 * String userName,String phoneNo) { // 주어진 주문 번호 (oNo), 회원 이름 (MemberName), 회원
	 * 전화번호 (memberPhoneNo)를 사용하여 주문을 조회하려면 적절한 로직을 추가 // orderRepository를 사용하여
