...
public class OrderDaoImpl implements OrderDao {
	...
	public Orders updateOrderByOState(String oState) {
		// Orders 테이블에서 oState를 기반으로 주문을 업데이트하려면 적절한 로직을 추가하세요.
		// orderRepository를 사용하여 업데이트할 주문을 찾아 업데이트하는 방식으로 구현 가능합니다.
		// orderRepository의 save 또는 update 메서드를 사용하여 주문 정보를 업데이트할 수 있습니다.
		return null;
	}
	...
	public List<Orders> findOrderWithOrderItemByMemberId(Long memberId) {
		// 주어진 memberId를 사용하여 해당 회원의 주문 목록을 조회하고, 주문 항목을 함께 로드하려면 적절한 로직을 추가하세요.
		// orderRepository와 JOIN 또는 FetchType.LAZY를 사용하여 관련된 주문 항목을 함께 로드할 수 있습니다.
		return null;
	}

	@Override
	public Orders findOrderByOrderNo(Long oNo) {
		// 주어진 주문 번호 (oNo)를 사용하여 해당 주문을 조회하려면 적절한 로직을 추가하세요.
		// orderRepository를 사용하여 데이터베이스에서 주문을 조회할 수 있습니다.
	...
	public Orders findOrderByOrderNoAndMemberNameandMemberPhoneNo(Long oNo, String MemberName, String memberPhoneNo) {
		// 주어진 주문 번호 (oNo), 회원 이름 (MemberName), 회원 전화번호 (memberPhoneNo)를 사용하여 주문을 조회하려면 적절한 로직을 추가하세요.
		// orderRepository를 사용하여 데이터베이스에서 주문을 조회할 수 있으며, 필요한 정보를 조건에 따라 조회할 수 있습니다.
