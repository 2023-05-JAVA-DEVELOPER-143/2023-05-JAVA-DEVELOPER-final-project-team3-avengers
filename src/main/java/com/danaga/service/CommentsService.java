...
public class CommentsService {
	...
	public List<CommentDto> comments(Long boardId){
		return cRepository.findByBoard_Id(boardId).stream()
				.map(comments -> CommentDto.createDto(comments)).collect(Collectors.toList());
	}
	@Transactional
	  public CommentDto createComment(CommentDto commentDto) {

	    
	    Board board = bRepository.findById(commentDto.getBoardId()).orElseThrow(() -> new IllegalArgumentException("존재하지 않는 게시물입니다."));
	    

	    Comments parent = null;
	    // 자식댓글인 경우
	    if(commentDto.getParentId() != null){
	      parent = cRepository.findById(commentDto.getParentId()).orElseThrow(() -> new IllegalArgumentException("존재하지 않는 상위 댓글입니다."));
	...
	public CommentDto update(Long id, CommentDto dto) {
		//댓글 조회 및 예외
		Comments target = cRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("댓글 생성 실패! 대상 게시글이 없습니다."));
	...
	public CommentDto delete(Long id) {
		//댓글 조회 및 예외
		Comments target = cRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("댓글 삭제 실패! 대상 댓글을 찾을수 없습니다."));
		//댓글을 DB에서 삭제
