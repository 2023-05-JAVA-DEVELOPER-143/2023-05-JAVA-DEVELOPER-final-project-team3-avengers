...
//
//	/*
//	 * @GetMapping() public String findCarts(HttpSession session) throws Exception {
//	 * // 로그인 유저로 찾기 sUserId = (String) session.getAttribute("sUserId"); // 회원이면
//	 * 아이디로 찾기 if (isLogin(sUserId)) { cartService.findCartList(sUserId); } // 비회원일시
//	 * 세션 리스트 fUserCarts = (List<CartCreateDto>) session.getAttribute("fUserCarts");
//	 * // 타임리프 문법으로 템플릿에서 하나씩 뿌려주기 return "리다이랙트 장바구니페이지"; }
//	 */
//
//	@GetMapping("/popup")
//	public String popup() {
...
//	/*
//	 * void addFUserCart(List<CartCreateDto> fUserCarts, CartCreateDto dto) throws
//	 * Exception { for (int i = 0; i < fUserCarts.size(); i++) { // 세션 장바구니의 리스트에
//	 * 똑같은 제품이 있으면 수량증가 if (dto.getOptionset() .getId() ==
//	 * fUserCarts.get(i).getOptionset().getId()) {
//	 * fUserCarts.get(i).setQty(fUserCarts.get(i).getQty() + dto.getQty()); } // 중복
//	 * 제품이 없다면 그냥 리스트에 추가 fUserCarts.add(dto); }
//	 * 
//	 * }
//	 */
//
