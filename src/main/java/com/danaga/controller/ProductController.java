...
public class ProductController {
	...
	public String productDetail(HttpSession session, @PathVariable Long optionSetId, Model model) {
		...
		if(service.updateViewCount(optionSetId).getMsg().equals(ProductExceptionMsg.FOUND_NO_OPTIONSET)) {
			log.warn("this optionset(id="+optionSetId+") already has found before but now failed to find and plus view count.");
		}
		...
		if (session.getAttribute("sUserId") != null) {// 로그인유저일시
			...
			try {
				...
				log.warn("login user(username="+session.getAttribute("sUserId")+") but failed to find in memberlist");
				e.printStackTrace();
			}
			ResponseDto<?> addRecentViewResult= recentViewService.addRecentView(RecentViewDto.builder().memberId(member.getId())// memberId
					.optionSetId(optionSetId).build());
			if(addRecentViewResult.getMsg().equals(ProductExceptionMsg.FOUND_NO_OPTIONSET)) {
				log.warn("this optionset(id="+optionSetId+") already has found before but now failed to find and plus view count.");
			}else if(addRecentViewResult.getMsg().equals(ProductExceptionMsg.FOUND_NO_MEMBER)) {
				log.warn("login user(username="+session.getAttribute("sUserId")+") but failed to find in memberlist");
			}
			// 최근본상품에 추가
			List<Boolean> isInterested;
			ResponseDto<?> interestResult= interestService.isMyInterest(optionSetId, username);
			if(interestResult.getMsg().equals(ProductExceptionMsg.FOUND_NO_OPTIONSET)) {
				log.warn("this optionset(id="+optionSetId+") already has found before but now failed to find and plus view count.");
			}else if(interestResult.getMsg().equals(ProductExceptionMsg.FOUND_NO_MEMBER)) {
				log.warn("login user(username="+session.getAttribute("sUserId")+") but failed to find in memberlist");
			}else if(interestResult.getMsg().equals(ProductSuccessMsg.IS_MY_INTEREST)) {
